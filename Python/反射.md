...
3. `inspect()`
    标准库 `inspect` 能精准反射出对象的相关信息：module、class、method、function、code、generator、coroutine、builtin。例如获取某个模块的所有函数：
    ```python
    import inspect
...
小心无限递归的陷阱：[`obj.name`, `getattr()`] <-> `__getattr__()`

### `__setattr__()`

小心无限递归的陷阱：[`obj.name = value`, `setattr()`] <-> `__setattr__()`
...
小心无限递归的陷阱：[`del obj.attr`, `delattr()`] <-> `__delattr__()`
