...
MySQL 提供了表锁和行锁两大类型的锁。表锁由 MySQL 服务器实现，无论你的存储引擎是什么都能使用表锁，而行锁是由特定的存储引擎实现，比如 InnoDB 默认是用行锁，
...
表锁是指对一整张表加锁，一般是 DDL 处理时使用，但是也可以在 DML 中手动指定
...
- InnoDB 默认使用的是行锁，只有在没有索引或者索引失效的情况下，才会退化成使用表锁
- InnoDB 对于读操作默认不加锁（但是也可以手动上锁），对于写操作默认加写锁
