...
1. 确保开启死锁日志、binlog
2. 通过死锁日志初步了解涉及的事务有哪些（一般是会有两个事务）、事务持有的锁是什么、等待的锁是什么、相关的 SQL 是什么（只保存事务阻塞前最新的一条）
3. 通过死锁日志中死锁发生的时间，或者 SQL，到 binlog 中找到其中一个事务的完整 SQL（注意，只有提交了的事务才会记录到 binlog，回滚的不会被记录，一般只会有一个阻塞事务的完整 SQL 被记录）
4. 根据这一组完整的 SQL 到应用代码中找到相关的逻辑，结合死锁日志中的一条 SQL，一般就可以推理出另一个事务在哪里
5. 尝试重现死锁的发生，根据实际情况调整并解决
...
这种方法可以实时监控并获取所有死锁信息，并写入 MySQL 的 stderr 日志，通过 `show variables like "%log_error%";` 可以找到日志的路径。
...
这种发发也是实时记录，而且相对第二种，只在死锁发生时记录，性能消耗更低
...
```SQL
1. begin;
3. select * from employees where emp_no=10001 for update;
5. select * from employees where emp_no=10002 lock in share mode;
```

T2
```SQL
2. begin;
4. select * from employees where emp_no=10002 for update;
6. select * from employees where emp_no=10001 lock in share mode;
```

sudo mysqlbinlog -h localhost -u root --read-from-remote-server mysql-bin.000001 --base64-output=decode-rows -v
