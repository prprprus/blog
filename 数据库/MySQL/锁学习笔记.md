...
- `WHERE` 的判断符号（范围运算符的加锁范围比等号运算符大得多）

2. 确定表锁还是行锁
3. 确定索引类型、是否需要回表（二级索引需要回表）
4. 确定回表需要加哪种锁（记录锁回表加记录锁、间隔锁回表不用加、next-key-lock 回表加记录锁）
...
1. 行锁，聚簇索引-写锁-记录锁
2. 行锁，聚簇索引-写锁-间隔锁
3. 行锁，二级唯一索引-写锁-记录锁，聚簇索引-写锁-记录锁
4. 行锁，二级唯一索引-写锁-间隔锁
5. 行锁，二级索引-写锁-next，聚簇索引-写锁-记录锁
6. 行锁，二级索引-写锁-间隔锁
7. 全表记录加 next
8. 行锁，聚簇索引-写锁-next
9. 行锁，二级索引-写锁-next，聚簇索引-写锁-记录锁
10. 行锁，二级索引-写锁-记录锁（SET 的字段），聚簇索引-写锁-记录锁
